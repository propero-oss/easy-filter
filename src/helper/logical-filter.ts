import { Filter, LogicalNotFilter, LogicalSetFilter } from "../types";

export function and(...filters: Filter[]): LogicalSetFilter;
export function and(filters: Filter[]): LogicalSetFilter;
export function and(...filterOrFilters: Filter[] | [Filter[]]): LogicalSetFilter {
  const filters =
    filterOrFilters.length > 0 ? (Array.isArray(filterOrFilters[0]) ? filterOrFilters[0] : (filterOrFilters as Filter[])) : [];
